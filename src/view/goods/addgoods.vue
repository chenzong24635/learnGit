<template>
  <div style="background:#fff;overflow:hidden;padding:30px;">
    ...
    <div class="page-title-add">创建商品</div>
    <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="120" >
      <FormItem  label="商品名称："  prop="goodsName">
        <Input v-model="formValidate.goodsName" placeholder="请输入商品名称" />
      </FormItem>
      <FormItem class="inline" label="商品总库存：" prop="goodsInventory">
      ...
      <br>
      <FormItem label="商品主图(有且只能有一张)" prop="mainImg">
        <upload-img @imgList="imgListMain" :isMultiple="false" :imgType="0" showImg></upload-img>
      </FormItem>
      <br>
      ...
      <br>
      <FormItem>
        <Button type="primary" style="margin-left:100px;width:100px;" @click="addGoods('formValidate')">创建商品</Button>
      </FormItem>
    </Form>
  </div>
</template>
  ...
  data () {
    ...
    return {
      ...
      ruleValidate: {
        ...
        mainImg: [
          { required: true, type: 'array', message: '请添加商品主图', trigger: 'change' }
        ],
        content: [
          { required: true, message: '请添加商品详情', trigger: 'blur' }
        ],
        banerImg: [
          { required: true, type: 'array', message: '请添加商品轮播图', trigger: 'change' }
        ]
      },
      content: '',
      editorOption: {},
